...
export default function AssessorVivaDashboard() {
  ...
  // Private Assessor Notes Drawer State
  ...
  const email = currentUser?.email || "assessor1@apiit.lk";
  ...
  const fetchAssignedSchedules = async () => {
    try {
      ...
      // Filter strictly for assessor role
      const myAssessorSchedules = (data.schedules || []).filter(s => s.isAssessor);
      setSchedules(myAssessorSchedules);
    } catch (err) {
  ...
  const handleOpenNotes = async (sch) => {
    ...
    try {
      const res = await fetch(`/api/viva/schedules/${sch.id}/notes?role=ASSESSOR`, { headers });
      if (res.ok) {
        const notes = await res.json();
        const myNote = notes.find(n => n.role === "ASSESSOR");
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const res = await fetch(`/api/viva/schedules/${notesSchedule.id}/notes`, {
        method: "POST",
        headers,
        body: JSON.stringify({ note: notesText, role: "ASSESSOR" })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to save notes");
      setToast({ type: "success", text: "Assessor Viva preparation notes saved." });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6 md:p-8">
      ...
      <div className="pb-6 border-b border-slate-200">
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Review Viva sessions where you are appointed as official Assessor. Confirm your attendance, prepare questions, and access student submissions.
        </p>
      </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20 text-slate-400 gap-2">
          ...
          <span>Loading your assigned Viva examinations...</span>
        </div>
      ) : error ? (
        ...
        <div className="mt-8 bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
          ...
          <h3 className="text-base font-bold text-slate-800">No Assessor Sessions Assigned</h3>
          <p className="text-xs text-slate-500 mt-1">
            You do not have any student Viva sessions assigned as assessor in the active Viva Period.
          </p>
        </div>
      ) : (
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-xs text-slate-600 bg-slate-50/60 p-3 rounded-xl border border-slate-100 mb-4">
                      ...
                      <div className="pt-1.5 border-t border-slate-200/60 text-[11px]">
                        <span><strong>Supervisor:</strong> {sch.supervisors?.name || "N/A"}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md p-6">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
              <div>
                <h3 className="text-base font-bold text-slate-900">Review Assessor Availability</h3>
            ...
            <p className="text-xs font-bold text-slate-800 mb-3">Are you available to assess this Viva?</p>
              ...
              {reviewAction === "CONFIRM" ? (
                <>
                  ...
                  <div>
                    ...
                    <textarea
                      rows={2}
                      placeholder="e.g. Confirmed for attendance."
                ...
                <>
                  <div>
                    ...
                    <textarea
                      rows={2}
                      required
                      placeholder="e.g. Prior academic commitment / lecture clash."
                      value={changeReason}
      ...
      {/* MODAL: PRIVATE ASSESSOR PREPARATION NOTES */}
      {notesSchedule && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg p-6">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-3">
              <div>
                <h3 className="text-base font-bold text-slate-900">Assessor Viva Preparation Notes</h3>
            ...
            <p className="text-xs text-slate-500 mb-3">
              🔒 <strong>Confidential Assessor Notes:</strong> Use this workspace to prepare your technical assessment questions, methodology evaluation points, and scoring reminders. Kept strictly independent from supervisor notes and invisible to students.
            </p>
            ...
            {notesLoading ? (
              ...
              <textarea
                rows={8}
                placeholder="Prepare assessment questions, methodology critique, verification requirements, technical queries..."
                value={notesText}
