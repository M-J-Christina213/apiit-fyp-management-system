            ...
            {periods.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                    {periods.map(period => {
                        ...
                        // Default to displaying as assessor if they are an assessor, else supervisor
                        const roleType = period.isAssessor ? 'assessor' : 'supervisor';
                        const activeId = roleType === 'supervisor' ? dashboardData.supervisorId : dashboardData.assessorId;
                        
                        return (
                            <div key={period.id}>
                                <div className="viva-card" style={{ marginBottom: '1rem', borderLeft: period.isAssessor ? '4px solid #f59e0b' : '4px solid #7c3aed' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
                                        <div>
