...
const AssessorVivaDashboard = () => {
    ...
    const fetchDashboard = async () => {
        try {
            const res = await fetch(`/api/viva/my-dashboard/assessor/${userId}`);
    ...
    const handleSaveAvailability = async (periodId, selectedSlots) => {
        try {
            const res = await fetch(`/api/viva/periods/${periodId}/availability`, {
                ...
                body: JSON.stringify({ assessor_id: userId, slots: selectedSlots })
            });
            if (res.ok) {
    ...
    return (
        <div className="viva-container">
            <div className="viva-header">
                <div>
                    <h1 className="viva-title">My Viva Schedules (Assessor)</h1>
                    <p className="viva-subtitle">Manage your Viva availability and view confirmed schedules for your assigned panels.</p>
                </div>
            </div>
                        ...
                        {schedules.map(sch => (
                            <div key={sch.id} className="viva-card" style={{ borderLeft: '4px solid #4f46e5' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginTop: '1.5rem' }}>
                                    <div>
                                        <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>Student</p>
                                        <p style={{ fontWeight: 500 }}>{sch.students?.student_name}</p>
                                    </div>
                                    <div>
            ...
            <div>
                ...
                {periods.length === 0 ? (
                    <p style={{ color: '#6b7280' }}>No active Viva periods for your assigned students.</p>
                ) : (
